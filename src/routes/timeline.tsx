...
import { useEvents, useRunJob } from '@/api/queries'
...
export default function Timeline() {
  ...
  const { data: serverEvents, isLoading, refetch } = useEvents()
  const { mutate: runJob, isPending: isProcessing } = useRunJob()

  // Sync server events to store
  useEffect(() => {
    if (serverEvents) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="glass px-4 py-2 rounded-lg text-cyan hover:text-white transition-colors disabled:opacity-50"
          >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${event.type === 'coding' ? 'bg-cyan/20 text-cyan' :
